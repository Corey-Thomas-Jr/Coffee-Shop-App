...
public class Expresso_Gui_Window {
        static int[] coffeeCount = {0};
        
        Expresso_Gui_Window() {
                // Window Frame
                        JFrame frame = new JFrame("Coffee Shop: Expresso List");
                ...
                // Options
                        JLabel Espresso_Label = new JLabel("Expresso options:");
                        Espresso_Label.setBounds(200, -1, 234, 44);
                        Espresso_Label.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 16));
                        Espresso_Label.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Espresso_Label);

                // Expresso Buttons
                        JButton Affogato_Button = new JButton("Affogato");
                        Affogato_Button.setBounds(1, 38, 214, 62);
                        Affogato_Button.setBackground(Color.decode("#2e2e2e"));
                        Affogato_Button.setForeground(Color.decode("#D9D9D9"));
                        Affogato_Button.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 14));
                        Affogato_Button.setBorder(new RoundedBorder(4, Color.decode("#979797"), 1));
                        Affogato_Button.setFocusPainted(false);
                        panel.add(Affogato_Button);

                        JButton Americano_Button = new JButton("Americano");
                        Americano_Button.setBounds(0, 136, 210, 61);
                        Americano_Button.setBackground(Color.decode("#2e2e2e"));
                        Americano_Button.setForeground(Color.decode("#D9D9D9"));
                        Americano_Button.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 14));
                        Americano_Button.setBorder(new RoundedBorder(4, Color.decode("#979797"), 1));
                        Americano_Button.setFocusPainted(false);
                        panel.add(Americano_Button);

                        JButton Mocha_Button = new JButton("Mocha");
                        Mocha_Button.setBounds(2, 231, 211, 70);
                        Mocha_Button.setBackground(Color.decode("#2e2e2e"));
                        Mocha_Button.setForeground(Color.decode("#D9D9D9"));
                        Mocha_Button.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 14));
                        Mocha_Button.setBorder(new RoundedBorder(4, Color.decode("#979797"), 1));
                        Mocha_Button.setFocusPainted(false);
                        panel.add(Mocha_Button);

                // Expresso Labels
                        JLabel Mocha_info = new JLabel(
                                        "Description: A Mocha is a rich, sweet espresso-based drink made with espresso, steamed milk, and chocolate syrup or cocoa powder, often topped  with whipped cream."); 
                                        // It’s essentially a chocolate-flavored latte, combining the richness of coffee and the sweetness of chocolate. It’s a popular choice // for those who enjoy a sweeter, dessert-like coffee.");
                        Mocha_info.setBounds(221, 237, 1000, 60);
                        Mocha_info.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 10));
                        Mocha_info.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Mocha_info);

                        JLabel Americano_info = new JLabel(
                                        "Description: An Americano is a simple yet flavorful coffee drink made by diluting a shot (or two) of espresso with hot water.");
                                        //This creates a coffee that is similar in strength to drip coffee but with a different taste profile. The espresso’s rich and bold flavor is preserved, but it's made less intense by the addition of hot water.");
                        Americano_info.setBounds(218, 138, 1000, 49);
                        Americano_info.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 10));
                        Americano_info.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Americano_info);

                        JLabel Affogato_info = new JLabel(
                                        "Description: a simple yet indulgent Italian dessert that combines espresso with ice cream. It typically consists of a shot of hot espresso poured over a scoop of vanilla ice cream.");
                                        // though variations may use other ice cream flavors or add toppings like chocolate shavings, nuts, or liqueurs. The contrast of  the hot, bold espresso and the cold, creamy ice cream creates a delightful  combination of flavors and textures.");
                        Affogato_info.setBounds(219, 50, 1000, 62);
                        Affogato_info.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 10));
                        Affogato_info.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Affogato_info);

                        JLabel Affogato_label = new JLabel("$4.00");
                        Affogato_label.setBounds(81, 21, 106, 17);
                        Affogato_label.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 14));
                        Affogato_label.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Affogato_label);

                        JLabel Americano_label = new JLabel("$2.50");
                        Americano_label.setBounds(85, 119, 106, 17);
                        Americano_label.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 14));
                        Americano_label.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Americano_label);

                        JLabel Mocha_label = new JLabel("$3.50");
                        Mocha_label.setBounds(88, 215, 106, 17);
                        Mocha_label.setFont(CustomFontLoader.loadFont("./resources/fonts/Lato.ttf", 14));
                        Mocha_label.setForeground(Color.decode("#D9D9D9"));
                        panel.add(Mocha_label);
                ...
                // Switch to Filtered list button
                        JButton Switch_Button = new JButton("Switch to Filtered Coffee");
        ...
        // This way only works for Windows
                ...
                String newline = "\n";
                        ...
                        // Write some text to the file
                                ...
                                // Affogato
                                        Affogato_Button.addActionListener(new ActionListener() {

                                                public void actionPerformed(ActionEvent e) {
                                                        coffeeCount[0]++;
                                                        selectedCoffeeLabel.setText("Selected: Affogato");
                                                        coffeeCountLabel.setText("Total: " + coffeeCount[0]);

                                                        Affogato_Expresso Affogato_object = new Affogato_Expresso("Affogato", "4.00", "a simple yet indulgent Italian dessert that combines espresso with ice cream.", "200", "6", "Whole milk", "Large scoop of ice cream" );
                                                        String a = Affogato_object.toString();

                                                        // This way only works for windows
                                                        // try {
                                                                // Files.write(file, newline.getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                                                                // Files.write(file, a.getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                                                                // Files.write(file, newline.getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                                 ...
                                 // Americano
                                        Americano_Button.addActionListener(new ActionListener() {
                                                public void actionPerformed(ActionEvent e) {
                                                        coffeeCount[0]++;
                                                        selectedCoffeeLabel.setText("Selected: Americano");
                                                        coffeeCountLabel.setText("Total: " + coffeeCount[0]);

                                                        Americano_Expresso Americano_object = new Americano_Expresso("Americano", "2.50", "An Americano is a simple yet flavorful coffee drink made by diluting a shot (or two) of espresso with hot water. This creates a coffee that is similar in strength to drip coffee but with a different taste profile.", "150", "4", "skim milk", "double shot: 4 calories" );
                                                        String a = Americano_object.toString();
                                 ...
                                 // Mocha
                                        Mocha_Button.addActionListener(new ActionListener() {
                                                public void actionPerformed(ActionEvent e) {
                                                        coffeeCount[0]++;
                                                        selectedCoffeeLabel.setText("Selected: Mocha");
                                                        coffeeCountLabel.setText("Total: " + coffeeCount[0]);

                                                        Mocha_Expresso Mocha_object = new Mocha_Expresso("Mocha", "3.50", "A Mocha is a rich sweet espresso-based drink made with espresso steamed milk and chocolate syrup or cocoa powder often topped with whipped cream. It’s essentially a chocolate-flavored latte combining the richness of coffee and the sweetness of chocolate. It’s a popular choice for those who enjoy a sweeter dessert-like coffee.", "250", "2", "almond milk", "can also have: whole milk or soy milk");
                                                        String a = Mocha_object.toString();
                        ...
                        // Switch button trigger to Filtered list
                                Switch_Button.addActionListener(new ActionListener() {
                                        public void actionPerformed(ActionEvent e) {
                                                frame.dispose();
                                                if (e.getSource() == Switch_Button) {
                                                        Filtered_Gui_Window E_Window = new Filtered_Gui_Window();
                                                }
                                        }
                                 });
